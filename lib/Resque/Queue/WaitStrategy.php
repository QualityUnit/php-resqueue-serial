...
class Resque_Queue_WaitStrategy implements Resque_Queue_EmptyQueueStrategy {
	...
	public function reserve() {
		...
		if(!$this->worker->isPaused()) {
			if($this->blocking === true) {
				...
				$this->worker->updateProcLine('Waiting for ' . implode(',', $this->worker->queues()->getQueues()) . ' with blocking timeout ' . $this->interval);
				$job = $this->worker->reserveBlocking($this->interval);
			} else {
				$this->worker->updateProcLine('Waiting for ' . implode(',', $this->worker->queues()->getQueues()) . ' with interval ' . $this->interval);
		...
		if(!$job) {
			...
			if($this->blocking === false)
			{
				...
				if($this->worker->isPaused()) {
					...
					$this->worker->updateProcLine('Waiting for ' . implode(',', $this->worker->queues()->getQueues()));
				}
